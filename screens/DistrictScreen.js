...
export default function DistrictScreen({ route, navigation}) {
  ...
  useEffect(() => {
    ...
    if (route.params?.eventsArrayVal) {
        eventsArray.push(route.params?.eventsArrayVal)
        console.log("Se ha actualizado los eventos", eventsArray)
    }
  }, [route.params?.eventsArrayVal]);
